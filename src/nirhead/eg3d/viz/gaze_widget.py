...
class GazeWidget:
    def __init__(self, viz):
        self.viz        = viz
        self.gaze       = dnnlib.EasyDict(yaw=0, pitch=0, anim=False, speed=0.25)
        self.gaze_def   = dnnlib.EasyDict(self.gaze)
    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            if dragging:
                self.drag(dx, dy)
            imgui.same_line()
            snapped = dnnlib.EasyDict(self.gaze, yaw=round(self.gaze.yaw, 1), pitch=round(self.gaze.pitch, 1))
            if imgui_utils.button('Snap', width=viz.button_w, enabled=(self.gaze != snapped)):
                self.gaze = snapped
            imgui.same_line()
            if imgui_utils.button('Reset', width=-1, enabled=(self.gaze != self.gaze_def)):
                self.gaze = dnnlib.EasyDict(self.gaze_def)
