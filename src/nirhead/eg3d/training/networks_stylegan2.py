...
class FullyConnectedLayer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        _weight = torch.randn([out_features, in_features]) / lr_multiplier
        _bias = torch.full([out_features], np.float32(bias_init)) if bias else None
        
        if trainable:
            self.weight = torch.nn.Parameter(_weight)
            self.bias = torch.nn.Parameter(_bias) if bias is not None else None
        else:
...
class MappingNetwork(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 attr_dim: int = 0,  # Custom static_attribute label dimensionality, 0 = no static_attributes label
                 ):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim
        ...
        if attr_dim > 0:
            self.attr_embed = FullyConnectedLayer(attr_dim, embed_features)
            self.attr_skip = FullyConnectedLayer(attr_dim, embed_features, trainable=False)
            self.reset_attr_parameters()
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, attr=None):
        ...
        with torch.autograd.profiler.record_function('input'):
            ...
            if self.c_dim > 0:
                ...
                if self.attr_dim > 0:
                    misc.assert_shape(attr, [None, self.attr_dim])
                    y_attr = self.attr_embed(attr.to(torch.float32))
                    y_attr = y_attr - self.attr_skip(attr.to(torch.float32))
                    y = y + y_attr
                y = normalize_2nd_moment(y)
                x = torch.cat([x, y], dim=1) if x is not None else y
            elif self.attr_dim > 0:
                misc.assert_shape(attr, [None, self.attr_dim])
                y = self.attr_embed(attr.to(torch.float32))
                y = y - self.attr_skip(attr.to(torch.float32))
                y = normalize_2nd_moment(y)
                x = torch.cat([x, y], dim=1) if x is not None else y
    ...
    def reset_attr_parameters(self):
        # torch.nn.init.eye_(self.attr_embed.weight)
        torch.nn.init.kaiming_uniform_(self.attr_embed.weight, a=math.sqrt(5))
        with torch.no_grad():
            self.attr_embed.bias.fill_(0.0)
            self.attr_skip.weight.copy_(self.attr_embed.weight)
            self.attr_skip.bias.copy_(self.attr_embed.bias)
