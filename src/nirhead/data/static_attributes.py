...
def labels_from_attribute_tensor(y_pred: torch.Tensor, file_keys: List[str], static_attributes: List[str]):
    ...
    for i in range(y_pred.shape[0]):
        ...
        for attr_idx, attr in enumerate(static_attributes):
            if not attr in labels[file_key] and types[attr].dim > 1:
                labels[file_key][attr] = []
            ...
            if types[attr].dim > 1:
                ...
                if types[attr].dtype == bool:
                    labels[file_key][attr] = tobool(y_pred[i][offset].item())
                else:
                    labels[file_key][attr] = types[attr].dtype(y_pred[i][offset].item())
                offset += 1
