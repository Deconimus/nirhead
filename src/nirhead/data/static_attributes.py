...
types_list = [
    ...
    StaticAttributeTypeInfo("eye_open",     "eo", float, 1, 0, 1),
]
...
def attributes_loss(attr_pred: torch.Tensor, attr_truth: torch.Tensor, static_attributes: List[str]):
    num_attributes = len(static_attributes)
    num_binary_attributes = len([attr for attr in static_attributes if types[attr].dtype == bool])
    
    lambda_binary = num_binary_attributes / num_attributes
    lambda_single = 1.0 / num_attributes
    ...
    loss = torch.nn.functional.binary_cross_entropy_with_logits(binary_attr_pred, binary_attr_truth) * lambda_binary
    ...
    for attr in static_attributes:
        ...
        if types[attr].dtype == float or types[attr].dtype == int:
            if dim > 1:
                loss_fun = torch.nn.functional.mse_loss # what did Euklid do to you, that you didn't just call it l2_loss?
            else:
                loss_fun = torch.nn.functional.l1_loss
                
        if loss_fun is not None:
            loss += loss_fun(attr_pred[:,idx_off:idx_off+dim], attr_truth[idx_off:idx_off+dim]) * lambda_single
...
def take_from_attribute_tensor(self, attributes_tensor_src, static_attributes_src, static_attributes_dst):
    ...
    idx = torch.tensor(attr_indices, device=attributes_tensor_src.device)
...
def take_binary_from_attribute_tensor(attr_tensor: torch.Tensor, static_attributes: List[str]):
    ...
    idx = torch.tensor(attr_indices, device=attr_tensor.device)
